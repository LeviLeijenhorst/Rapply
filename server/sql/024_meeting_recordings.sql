create table if not exists public.meeting_recordings (
  ...
  session_id text not null references public.inputs (id) on delete cascade,
  ...
  constraint meeting_recordings_status_check check (
...
create index if not exists meeting_recordings_audio_stream_id_idx on public.meeting_recordings (audio_stream_id);
...
create index if not exists meeting_recording_tokens_lookup_idx
  on public.meeting_recording_tokens (owner_user_id, meeting_recording_id, expires_at, used_at);
