create table if not exists public.practice_settings (
  ...
  constraint practice_settings_practice_name_length check (char_length(practice_name) <= 200),
  constraint practice_settings_website_length check (char_length(website) <= 320),
  constraint practice_settings_tint_color_length check (char_length(tint_color) <= 20),
