...
export async function ensureDefaultExampleDataForUser(userId: string): Promise<void> {
  ...
  await execute(
    `
    insert into public.clients (id, owner_user_id, name, client_details, employer_details, created_at_unix_ms, updated_at_unix_ms, is_archived)
  ...
  for (let index = 0; index < janVoorbeeldSessions.length; index += 1) {
    ...
    await execute(
      ...
      values ($1, $2, $3, null, $4, $5, 'session_report', 'needs_review', $6, $7, $8)
      on conflict (id) do nothing
      `,
      [`report-example-${userId}-jan-voorbeeld-${index + 1}`, userId, clientId, sessionId, item.title, item.summary, item.reportDate, sessionUnixMs, sessionUnixMs],
    )
  }
}
