...
export async function bootstrap(params: { userId: string; deviceId: string }): Promise<{ e2eeEnabled: boolean; wrappedUserDataKeyForDevice: string | null; recoveryKeyUpdatedAtMs: number | null }> {
  const userRow = await queryOne<{ wrapped_user_data_key_for_recovery: string; recovery_key_updated_at_unix_ms: number }>(
    "select wrapped_user_data_key_for_recovery, recovery_key_updated_at_unix_ms from public.e2ee_users where user_id = $1",
    [params.userId],
  )

  if (!userRow) {
    return { e2eeEnabled: false, wrappedUserDataKeyForDevice: null, recoveryKeyUpdatedAtMs: null }
  }
...
export async function setupE2ee(params: {
  userId: string
  deviceId: string
  wrappedUserDataKeyForDevice: string
  wrappedUserDataKeyForRecovery: string
  nowUnixMs: number
}): Promise<void> {
  await execute(
    `
      insert into public.e2ee_users (user_id, wrapped_user_data_key_for_recovery, recovery_key_updated_at_unix_ms)
      values ($1, $2, $3)
