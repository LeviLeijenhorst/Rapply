...
import {
  ...
  splitCoacheeName,
  toPlaceholderKey,
} from '@/screens/report/report.logic'
...
import { useReportScreenProps, useReportScreenState } from '@/screens/report/report.hooks'
import type {
  ActivityAllocationRow,
  UwvField,
  UwvFieldGroup,
} from '@/screens/report/report.types'
...
export function ReportScreen(props: ReportScreenProps) {
  const { initialCoacheeId, initialSessionId, mode } = useReportScreenProps(props)
  ...
  const {
    ...
    selectedTemplateId, setSelectedTemplateId, werkfitTemplates,
    selectedSessionIds, setSelectedSessionIds, selectedRapportageIds, setSelectedRapportageIds,
  ...
  selectedCoachee, selectedTrajectory, sessionRows, rapportageRows, noteRows, selectedTemplate, uwvFields, groupedFields,
  specialistTariffAnswerByGroup, activeRows, activeSelectedSet, areAllActiveRowsSelected, canStartGeneration,
} = useReportScreenState({ data, initialCoacheeId, initialSessionId })
const assistantScrollRef = useRef<ScrollView | null>(null)
  ...
  const handleExportWord = async () => {
    try {
      ...
      console.error('[ReportEditorScreen] UWV Word export failed', error)
