...
import { decryptAudioChunkFromStorage, decryptAudioFromStorage, decryptAudioStreamFromStorage, encryptAudioChunkForStorage, encryptAudioForStorage } from '../encryption/audioCrypto'
import {
  createRecoveryKey,
...
import { createArgon2SaltBase64Url, defaultArgon2Params, derivePassphraseKeyBytes } from '../crypto/passphraseKdf'
import {
  e2eeBootstrap,
  ...
  async function handleSetupPassphrase() {
    ...
    try {
      ...
      if (setupMode === 'new') {
        ...
        const finalUserDataKeyBytes = userDataKeyBytes
        ...
        const wrappedArkUserPassphrase = await encryptBytesWithAesGcm({ key: passphraseKey, plaintext: finalUserDataKeyBytes })
        const wrappedArkRecoveryCode = await wrapUserDataKeyForRecovery({ recoveryKey: nextRecoveryKey, userDataKeyBytes: finalUserDataKeyBytes })
        ...
        await saveRememberedArk({ keyVersion: nextKeyVersion, arkBytes: finalUserDataKeyBytes })
        const nextUserDataKey = await importAesKey(finalUserDataKeyBytes)
        setUserDataKey(nextUserDataKey)
        setIsConfigured(true)
        setIsEnabled(true)
