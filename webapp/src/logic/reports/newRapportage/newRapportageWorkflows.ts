...
import type { LocalAppData, Template } from '../../../local/types'
import { exportReportWord, generateReportSummary } from '../../../api/reports'
import { normalizeSummaryTemplate } from '../../../services/summary'

export type RapportageInputField = {
  key: string
  label: string
  rawLabel: string
}

export type RapportageFieldGroup = {
  title: string
  fields: RapportageInputField[]
}
...
export function buildRapportageGenerationSourceText(params: {
  data: LocalAppData
...
export async function generateRapportageText(params: {
  selectedTemplate: Template
  sourceText: string
  ...
  let generatedReport = ''
  if (sourceText) {
    generatedReport = await generateReportSummary(sourceText, normalizeSummaryTemplate(selectedTemplate))
  }
  return generatedReport || fallbackReportText
}

export async function exportRapportageWord(params: {
  templateName: string
  reportText: string
  contextValues: Record<string, string>
}): Promise<boolean> {
  return exportReportWord({
...
export function buildAssistantReportContext(params: {
  groupedFields: RapportageFieldGroup[]
...
export async function sendRapportageAssistantMessage(params: {
  chatMessages: Array<{ role: 'user' | 'assistant'; text: string }>
  reportContext: string
}): Promise<string> {
  const modelMessages: ApiChatMessage[] = [
    {
      role: 'system',
      text:
    ...
    ...params.chatMessages.map<ApiChatMessage>((message) => ({ role: message.role, text: message.text })),
  ]

  return sendAiChat(modelMessages)
}
