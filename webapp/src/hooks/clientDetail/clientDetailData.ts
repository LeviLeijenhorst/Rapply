import type { LocalAppData, Session, Trajectory } from '../../storage/types'
...
export function getClientTrajectories(data: LocalAppData, clientId: string): Trajectory[] {
  return data.trajectories.filter((item) => item.coacheeId === clientId).sort((a, b) => a.createdAtUnixMs - b.createdAtUnixMs)
}

export function getActiveTrajectory(clientTrajectories: Trajectory[], preferredTrajectoryId: string): Trajectory | null {
  return preferredTrajectoryId
    ? clientTrajectories.find((trajectory) => trajectory.id === preferredTrajectoryId) ?? clientTrajectories[0] ?? null
    : clientTrajectories.find((trajectory) => Boolean(String(trajectory.startDate || '').trim())) ?? clientTrajectories[0] ?? null
}

export function getClientSessionListItems(data: LocalAppData, clientId: string): {
  ...
  const allSessionItems = data.sessions
    .filter((item) => item.coacheeId === clientId)
  ...
  const notesSessionIds = new Set(
    data.sessions.filter((session) => session.coacheeId === clientId && isSessionNotesArtifact(session)).map((session) => session.id),
  )
  ...
  const notesSession =
    data.sessions
      .filter((session) => session.coacheeId === clientId && isSessionNotesArtifact(session))
